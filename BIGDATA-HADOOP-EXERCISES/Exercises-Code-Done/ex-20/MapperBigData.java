...
class MapperBigData extends Mapper<
	...
	private MultipleOutputs<Text, NullWritable> multiOutSample = null;
	float threshold;
	
	protected void setup(Context context)
	{
		// Create a new MultiOuputs using the context object
		multiOutSample = new MultipleOutputs<Text, NullWritable>(context);
		threshold= 30;
	}
    ...
    protected void map(
			...
			if (temp > threshold) {
				multiOutSample.write("hightemp",value,NullWritable.get());
			}
			else {
				multiOutSample.write("normaltemp", value, NullWritable.get());
			}
